		...
		.equ FREQ = 8000000			;???????? ?????????? ??????, ??
		.equ FreqSCL = 400000			;????????? ???????? ?????? i2c, ??
		.equ FreqTWBR = ((FREQ/FreqSCL)-16)/2	;???????????? ???????? ???????? TWBR
;======================================== Start macro.inc==========================
.MACRO SETB				;?????? ????????? ????
.if @0 < 0x20 ; Low IO
SBI @0,@1
...
SCND_START:	RCALL TWI_Stop				;??? ?????????? ?????? ???? ???? ????
		ldi try,3
		RJMP Vix			
SLAW_Adr:	MOV temp,addr			;???????? ????? ????????(??????? ????? ? ???????? addr ? ??????? temp
		;INC temp
		RCALL TWI_SendByte		;????????? ?? ???????????? ??? ???????? ??????
		RJMP Vix
SLA_W_ACK:	LD temp,-Y			;?????????? ???????? ?????? ?????? ?????? ?????????? ??? ??????? ?????????? ? ??????? temp
		inc sys				;?????????????? ??????? ?????????? ????
		RCALL TWI_SendByte		;????????? ?? ???????????? ??? ???????? ??????
		RJMP Vix
...
Stopp:	LDI temp, TWDR
		...
		LDI try,2
		RJMP Vix
TWI_SendByte:	STS TWDR,temp			;????????? ???????? ???????? temp ? ??????? ??????
		LDI temp, 0b10000101		;????????????? ????????? ??? ??????????? ?????? i2c ????? (TWINT=1, TWEN=1, TWIE=1)? ??????? temp
		STS TWCR,temp			;????????? ?????? ?? ???????? temp ? ??????? TWCR
		RET				;??????? ?? ????????????
NextByteSend:	CPI sys,8			;????????? ??? ?? ?? 8-?? ???? ?????????? ?? ?????????.
		BREQ ByteSendStop		;???? ??, ?? ????????? ?? ???????????? ??? ???????? ?????
		LD temp,-Y			;???? ??? ???????? ?????????????? ?????, ?? ????????? ????????? ???? ?????????? ? temp
		RCALL TWI_SendByte		;? ???????? ???????????? ??? ??? ????????
		inc sys				;?????????????? ???????? ???????? ????????????? ????
...
TWI_ReceiveAck: LDI temp, 0b11000101
		STS TWCR, temp
		RET
TWI_Stop:	LDI temp,0b10010100		;????????????? ????????? ??? ??????????? ?????? i2c ????? (TWINT=1, TWSTO=1, TWEN=1)? ??????? temp
		STS TWCR,temp		;????????? ?????? ?? ???????? temp ? ??????? TWCR
		RET
...
I2C_SEND:ldi sys,0
		ldi try,1
		ldi YL, low(VAR_SLV_1)		;???????? ? ?????? ????????? Y ????? ?????? ?????? ?????? ?????????? ??? ?????????? VAR_SLV_1
...
I2C_RCV: ldi sys,0
		ldi try,1
		ldi YL, low(VAR_SLV_2)		;???????? ? ?????? ????????? Y ????? ?????? ?????? ?????? ?????????? ??? ?????????? VAR_SLV_1
		ldi YH, high(VAR_SLV_2)
		ldi temp, 0b10100101		;TWINT=1, TWSTA=1, TWEN=1, TWIE=1
		STS TWCR,temp			;???? ?????
		SEI				;????????? ??????????(?????????)
Delay2:	CPI try,3
...
Left:		CLI
		ldi addr, 0x32
		ldi temp,0b00010000		;????????? ?????? ? ?????? ?? ????? VAR_SLV_2
		RCALL I2C_SEND
		;RCALL I2C_RCV
		RJMP MAIN
Stop:		CLI
		ldi addr, 0x32
		ldi temp,0b00000000		;????????? ?????? ? ?????? ?? ????? VAR_SLV_2
		;RCALL I2C_RCV
		;dec addr
		RCALL I2C_SEND
		RJMP MAIN
